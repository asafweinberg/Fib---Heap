...
//                FiboHeapPrinter.printHeap(heap);
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int size = heap.size(), numOfTrees = heap.getTreesNumber(), numMarked = getNumMarked(heap);
        heap.meld(heap2);
        if (heap.size() != size + heap2.size()) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.getTreesNumber() != numOfTrees + heap2.getTreesNumber()) {
    ...
    public static void testKMin(FibonacciHeap heap) {
        if (heap.size() == 0) {
            ...
            FibonacciHeap heaps[] = new FibonacciHeap[heap.getTreesNumber()];
            int heapIndex = 0;
            do {
                next = cur.getNext();
                cur.setPrev(cur);
    ...
    public static void testNumOfTrees(FibonacciHeap heap) {
        ...
        if (actualNumOfTrees != heap.getTreesNumber()) {
            throw new RuntimeException(String.format("should have %s trees but counted %s", heap.getTreesNumber(), actualNumOfTrees));
        }
    }
    ...
    public static void testHeapFields(FibonacciHeap heap, ExpectedFields expected) {
        if (expected.numOfTrees != null && expected.numOfTrees != heap.getTreesNumber()) {
            throw new RuntimeException(
                    String.format("wrong amount of trees. found %d but expected %d",
							heap.getTreesNumber(), expected.numOfTrees)
            );
        }
