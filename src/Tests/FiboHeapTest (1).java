...
public class FiboHeapTest {
    ...
    public static int getNumMarked(FibonacciHeap heap) {
        return (heap.potential() - heap.treesCounter) / 2;
    }
...
//                FiboHeapPrinter.printHeap(heap);
    ...
    public static void testMeld(FibonacciHeap heap, int upperBound) {
        ...
        int size = heap.size(), numOfTrees = heap.treesCounter, numMarked = getNumMarked(heap);
        heap.meld(heap2);
        if (heap.size() != size + heap2.size()) {
            throw new RuntimeException("size after meld is not sum of both heaps");
        }
        if (heap.treesCounter != numOfTrees + heap2.treesCounter) {
    ...
    public static void testKMin(FibonacciHeap heap) {
        if (heap.size() == 0) {
            ...
            FibonacciHeap.HeapNode cur = heap.findFirst();
            FibonacciHeap.HeapNode next;
            Random rand = new Random(0L);
            int kMin;
            int[] kMinVals, dumbKMinVals;
            FibonacciHeap heaps[] = new FibonacciHeap[heap.treesCounter];
            int heapIndex = 0;
            do {
                next = cur.getNext();
                cur.setPrev(cur);
                ...
                heapIndex++;
                cur = next;
            } while (next != null && next != heap.findFirst());
    ...
    public static int calculateNumOfTrees(FibonacciHeap heap) {
        ...
        FibonacciHeap.HeapNode cur = heap.findFirst().getNext();
        while (cur != heap.findFirst()) {
            i++;
            cur = cur.getNext();
        }
    ...
    public static void testNumOfTrees(FibonacciHeap heap) {
        ...
        if (actualNumOfTrees != heap.treesCounter) {
            throw new RuntimeException(String.format("should have %s trees but counted %s", heap.treesCounter, actualNumOfTrees));
        }
    }
    ...
    public static void testHeapFields(FibonacciHeap heap, ExpectedFields expected) {
        if (expected.numOfTrees != null && expected.numOfTrees != heap.treesCounter) {
            throw new RuntimeException(
                    String.format("wrong amount of trees. found %d but expected %d",
                            heap.treesCounter, expected.numOfTrees)
            );
        }
